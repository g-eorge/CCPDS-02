...
drgplot = plot(df, x = :ICD9, y = :Average_Total_Payments, Geom.point, Scale.x_discrete)
draw(PNG(cwd * "/plots/Procedure_Cost_Gadfly.png", 12inch, 4inch), drgplot)

# Compute the variance by ICD9 code
variance = by(df, :ICD9, df -> DataFrame(Variance = var(df[:Average_Total_Payments])))

# Get the top 3 codes
sortby!(variance, :Variance, Order.Reverse)
top3 = DataFrame(variance[1:3,:ICD9])
